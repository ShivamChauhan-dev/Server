...
router.put('/projects/:project_name', async (req, res) => {
...
router.delete('/projects/:project_name', async (req, res) => {
  const projectId = req.params.id;
  try {
    const result = await pool.query('DELETE FROM projects WHERE id = $1 RETURNING project_name', [projectId]);
